...
export const tablasController = {
  insertarVenta: async (req, res) => {
    ...
    try {
      console.log(req);
      const { idvendedor, idcliente, cantidadnormal, cantidadrapida, cantidaddinero, numerotransaccion } = req.body;
      console.log(idvendedor);
      const tempor = await pool.query(
        `INSERT INTO venta(id_vendedor, id_cliente, fecha, cantidad_normal, cantidad_rapida, cantidad_dinero, numero_transaccion)
                    values(${idvendedor}, ${idcliente}, CURRENT_TIMESTAMP,${cantidadnormal}, ${cantidadrapida}, ${cantidaddinero}, '${numerotransaccion}')`
      );
